...
metadata {
	definition (name: "Anidea Ikea Button", namespace: "smartthings", author: "SmartThings", ocfDeviceType: "x.com.st.d.remotecontroller", mcdSync: true) {
		capability "Actuator"
		capability "Battery"
		capability "Button"
		...
		fingerprint manufacturer: "IKEA of Sweden", model: "TRADFRI SHORTCUT Button", deviceJoinName: "IKEA Remote Control", mnmn: "SmartThingsCommunity", vid: "bcf8ccdc-ad91-3e3b-9397-78c018ad7f2b" // raw description 01 0104 0820 01 06 0000 0001 0003 0009 0020 1000 07 0003 0004 0006 0008 0019 0102 1000
...
private getIkeaShortcutButtonNames() {
	[
    	"shortcut button"
    ]
}
...
private getButtonLabel(buttonNum) {
	...
	} else if (isIkeaShortcutButton()) {
    	label = ikeaShortcutButtonNames[buttonNum - 1]
	...
	if (isIkeaRemoteControl()) {
		...
		buttonNumber = event.buttonNumber
 		...
 		if (descMap.clusterInt == zigbee.ONOFF_CLUSTER) {
			buttonState = "pushed"
			buttonNumber = SHORTCUT_BUTTONS.SHORTCUT
...
private boolean isIkeaShortcutButton() {
	device.getDataValue("model") == "TRADFRI SHORTCUT Button"
}
...
private boolean isIkea() {
	isIkeaRemoteControl() || isIkeaOnOffSwitch() || isIkeaOpenCloseRemote() || isIkeaShortcutButton()
}
