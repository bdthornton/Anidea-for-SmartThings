...
metadata {
	...
	definition (name: "Anidea for ZigBee Button", namespace: "smartthings", author: "Mitch Pond", ocfDeviceType: "x.com.st.d.remotecontroller") {
        capability "Actuator"
        capability "Battery"
        capability "Button"
    	...
    	fingerprint manufacturer: "eWeLink", model: "WB01", deviceJoinName: "eWeLink Button" // Sonoff SNZB-01
    }
...
private Map getBatteryResult(rawValue) {
    ...
    // WB01 seems to be returning 3.8 V for a new battery.
...
private Map parseNonIasButtonMessage(Map descMap){
    ...
    if ((device.getDataValue("model") == "3460-L") &&(descMap.clusterInt == 0x0006)) {
        ...
        return createEvent(name: "button", value: buttonState, isStateChange: true)
    }
...
def initialize() {
    ...
    else if ((device.getDataValue("manufacturer") == "eWeLink") && (device.getDataValue("model") == "WB01")){
            sendEvent(name: "numberOfButtons", value: 1, displayed: false)
            sendEvent(name: "supportedButtonValues", value: ["pushed", "pushed_2x", "held"].encodeAsJSON(), displayed: false)
        }
    else {
